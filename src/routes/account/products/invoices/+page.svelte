...
<Table.Root class="text-md overflow-hidden rounded-md bg-muted text-center font-mono">
	...
	<Table.Header class="bg-primary-700">
		<Table.Row>
			...
			<Table.Head class="w-0 p-0 text-right"></Table.Head>
		</Table.Row>
	</Table.Header>
	...
	<Table.Body class="font-mono">
		{#each data as row}
			<Table.Row>
				...
				<Dialog.Root>
					<Dialog.Trigger class="m-0 flex h-full w-full items-center justify-center p-0 pr-2">
						<Table.Cell class="px-0 "><Info class="m-0 w-5 text-muted-foreground" /></Table.Cell>
					</Dialog.Trigger>
							...
							<Dialog.Description>
								<div class="flex flex-col gap-5">
									<div class="flex flex-col">
										<p>Title</p>
										<h3 class=" text-lg font-bold text-foreground">{row.item}</h3>
									</div>
									<div class="flex justify-between">
										<div class="flex flex-col">
											<p>Issued</p>
											<h3 class="font-bold text-foreground">{row.date}</h3>
										</div>
										<div class="flex flex-col">
											<p>Due Date</p>
											<h3 class="font-bold text-foreground">{row.due_date}</h3>
										</div>
										<div class="flex flex-col">
											<p>Invoice No.</p>
											<h3 class="font-bold text-foreground">{row.id}</h3>
										</div>
									</div>
									<div class="flex justify-between">
										<div class="flex flex-col">
											<p>Status</p>
											<h3 class="font-bold text-foreground">{row.status}</h3>
										</div>
										<div class="flex flex-col">
											<p>Payment Method</p>
											<h3 class="font-bold text-foreground">{row.payment_method}</h3>
										</div>
										<div class="flex flex-col">
											<p>Price</p>
											<h3 class="font-bold text-foreground">{row.amount}</h3>
										</div>
									</div>
									<div class="flex justify-end">
										<div class="px-2 py-2">
											<Button
												class="flex h-full items-center justify-center border-transparent bg-foreground focus:border-transparent focus:ring-0"
												href=""
