...
<div class="fixed bottom-0 w-full border-t-1 grid grid-rows-1 grid-cols-4 gap-2 p-1">
      {#each links as link (link)}
        ...
        <a href={link.href} class={['btn flex-col items-center flex justify-center p-2 rounded-lg gap-1 text-white', page.url.pathname === link.href && "bg-primary-600"]} style="filter: brightness(100%)">
          <Icon class="size-5" />
